...
minimizing the distance between pairs. All of this is controlled by setting 
`match = FALSE` in `tween_polygon()`, and `transformr` will then do its magic:
...
minimize the difference in length, not minimizing the pair distance. This has 
been chosen as extreme changes in path length are deemed to be more confusing
than long traveling distance. The same interpretation of the `enter`, `exit`, 
and `match` arguments remain, which can be seen in the two examples below:
...
## Future
The main goal right now is to get `transformr` ready for a release on CRAN. 
Further development will be guided by the needs of the community, so open an
issue if you have any unmet needs or cool ideas!
